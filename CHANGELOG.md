...
- **11 个新测试**（`tests/test_console_assets.py`）。校验命令组与别名、帮助文案、`app.js` 引用的元素 id 在 HTML 中都存在、6 套皮肤样式齐全且玻璃皮肤确有毛玻璃与荧光声明、两种密度都有样式、页签与面板集合一致、中英文案完全对齐。
...
- **i18n 重建**。页面文案抽为 166 个 key，`zh-CN.json` 与 `en-US.json` 完全对齐，缺失时回落到内置默认文案。
